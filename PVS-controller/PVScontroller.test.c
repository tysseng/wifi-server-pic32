...
void testThatNoteOffUpdatesReadyToSendOffBit(){
    // given
    checkKeyStartSwitches(0b01000000, 0, 0); // turns note on
    ...
    checkKeyStartSwitches(0b00000000, 0, 0); // same state
...
void testThatCorrectTimerBytesAreSet(){
    checkKeyStartSwitches(0b10000000, 0, 0); // note 7 * 8 + 0 = 56
    assertEquals(2, noteTimers[56], "Note 56 not set");
    
    checkKeyStartSwitches(0b01000000, 1, 0); // note 6 * 8 + 1 = 49
    assertEquals(2, noteTimers[49], "Note 49 not set");
    
    checkKeyStartSwitches(0b00100000, 2, 0); // note 5 * 8 + 2 = 42
    assertEquals(2, noteTimers[42], "Note 42 not set");
    
    checkKeyStartSwitches(0b00010000, 3, 0); // note 4 * 8 + 3 = 35
    assertEquals(2, noteTimers[35], "Note 35 not set");
    
    checkKeyStartSwitches(0b00001000, 4, 0); // note 3 * 8 + 4 = 28
    assertEquals(2, noteTimers[28], "Note 28 not set");
    
    checkKeyStartSwitches(0b10000100, 5, 0); // note 2 * 8 + 5 = 21
    assertEquals(2, noteTimers[21], "Note 21 not set");
    
    checkKeyStartSwitches(0b10000010, 6, 0); // note 1 * 8 + 6 = 14
    assertEquals(2, noteTimers[14], "Note 14 not set");
    
    checkKeyStartSwitches(0b10000001, 7, 0); // note 0 * 8 + 7 = 7
...
void testThatNotesAbove60AreIgnoredInCalcNoteOnOff(){
    ...
    checkKeyStartSwitches(0b00000000, 5, 0); //simulate all keys released
    ...
    checkKeyStartSwitches(0b00000000, 6, 0); //simulate all keys released
    ...
    checkKeyStartSwitches(0b00000000, 7, 0); //simulate all keys released
...
void testVelocityCalculationNormal(){
    ...
    checkKeyEndSwitches(0b00000001, 0, 0); // simulate key strike end
...
void testThatReadyToSendIsNotRetriggered(){
    ...
    checkKeyEndSwitches(0b00000001, 2, 0); // same state (key has not been released
